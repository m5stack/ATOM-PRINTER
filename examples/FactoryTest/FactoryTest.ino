...
void flashing(uint32_t color,
              uint8_t frequency); // rgb灯的闪烁 color：颜色 frequency：频率(ms)
...
void setup() {
  ...
  /*打印机模式选择：
    当flash没有保存wifi信息：1.当前状态为AP -> 保持APMode
                            2.当前状态为UIFLOW ->
    切换至APMode，并提示需要输入wifi信息 当flash保存了wifi信息  ：1.当前状态为AP
    -> 保持APMode 2.当前状态为UIFLOW -> 保持UIFLOW
  */
...
void mqttCallback(char *topic, byte *payload, unsigned int len) {
  ...
  //   {
  //     atomPrinter.Printer_Init();
  // //    atomPrinter.Sel_Align(0x01);
  //     atomPrinter.Print_BitMap(0,184,180,bitbuffer2);
  //     atomPrinter.Print_NewLine(3);
  //   }
}
...
void handleRoot() {
  String s = "<html><head><meta  charset=\"gbk\" name=\"viewport\" "
             "content=\"width=device-width,user-scalable=0\"><div "
             "align=\"center\"><title>ATOM_Printer</title></head>";
  s += "<body><h1>ATOM Printer</h1><p></p><iframe id=\"id_frame\" name = "
       "\"id_frame\" style=\"display:none;\"></iframe><form id=\"print\" "
       "method=\"get\" action=\"\" target=\"id_frame\"><p><input "
       "type=\"radio\" name=\"printType\" value=\"ASCII\"><font "
       "size=\"6\"><strong>ASCII</strong></font><br><input id=\"Pdata\" "
       "length=\"256\" type=\"text\" name=\"Pdata\" "
       "onkeyup=\"value=value.replace(/[^\w\u4E00-\u9FA5]/g,'')\"></p>";
  s += "<p><input type=\"radio\" name=\"printType\" value=\"QRCode\"><font "
       "size=\"6\"><strong>QRCode</strong></font><br><strong>Adjust "
       "Level:</strong><select "
       "name=\"AdjustLevel\"><option>L</option><option>M</option><option>Q</"
       "option><option>H</option></select><br><input id=\"QRCode\" "
       "length=\"256\" type=\"text\" name=\"QRCode\" "
       "onkeyup=\"value=value.replace(/[^\w\u4E00-\u9FA5]/g,'')\"></p>";
  s += "<p><input type=\"radio\" name=\"printType\" value=\"BarCode\"><font "
       "size=\"6\"><strong>BarCode</strong></font><br><strong>Set "
       "HRI:</strong><select "
       "name=\"Position\"><option>Hide</option><option>Above</"
       "option><option>Below</option><option>Both</option></select><br>";
  s += "<input id=\"BarCode\" maxlength=\"18\" type=\"text\" name=\"BarCode\" "
       "onkeyup=\"value=value.replace((/[\W]/g,'')\"</p>";
  s += "<p><input type=\"checkbox\" name=\"newLine\" value=\"on\">newLine</p>";
  s += "<p><input type=\"submit\" value=\"print\"></p>";
  s += "</form>";
  s += "<h1>Wi-Fi Settings For UIFLOW</h1><p>Please enter your password by "
       "selecting the SSID.</p>";
  s += "<form id=\"wifi\" method=\"get\" action=\"/Connect\"><label>SSID: "
...
void setWebServer() {
  if (printerMode == APMode) {
    ...
    webServer.on("/", []() {
      ...
      s += "<form method=\"get\" action=\"/Connect\"><label>SSID: "
           "</label><select name=\"ssid\">";
      s += ssidList;
      s += "</select><br>Password: <input name=\"pass\" length=64 "
           "type=\"password\"><input type=\"submit\"></form>";
    ...
    webServer.on("/switch", []() {
      ...
      preferences.putBool("PRINTER_MODE", APMode);
      String s =
          "<h1>Printer has change to APMode</h1><p>Please reset device.</p>";
      webServer.send(200, "text/html", makePage("Reset Wi-Fi Settings", s));
      delay(2000);
      ESP.restart();
    });
...
void setWifiMode() {
  if (printerMode == APMode) {
    ...
    String No = MAC.substring(MAC.length() - 5, MAC.length() - 3) +
    ...
    Serial.println(
        WiFi.localIP()); // IP address assigned to your ESP  获取ip地址

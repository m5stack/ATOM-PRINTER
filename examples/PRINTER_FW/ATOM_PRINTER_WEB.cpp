...
void handleState() {
    ...
    if(WiFi.status() == WL_CONNECTED){
        ...
        if(mqttClient.connected()) {
            payload["MQTT_STATE"] = "Server:" + mqtt_broker + "\r\nSubscribe Topic: " + mqtt_topic;
        }else{
...
void handleMQTTConfig() {
    ...
    if(mqttConnect(
        ...
        2000)) 
    {
       webServer.send(200, "text/html", "Server:" + mqtt_broker + "\r\nSubscribe Topic: " + mqtt_topic);
    } else {
       webServer.send(200, "text/html", "ERROR");
    }
...
void handleBMP(){
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        Serial.print("handleFileUpload Size: "); Serial.println(upload.currentSize);
        size_t size = upload.currentSize;
        memcpy(bmp_buffer+bmp_data_offset, upload.buf, size);
        bmp_data_offset += size;
    } else if (upload.status == UPLOAD_FILE_END) {
        Serial.print("handleFileUpload totalSize: "); Serial.println(upload.totalSize);
